...
- **`pre_built/build_scripts/test-prebuilt-binaries` is the release gate.** Run `./release --dry-run` before tagging. The script does a full `./install --dest-dir /tmp/...`, probes each binary for loader failures and signal crashes, checks editor runtime sentinels, runs installed `nvim` headless against the installed runtime, and asserts portable Python does not shadow `python3`/`pip3`.
...
- Linux installer manages Starship at `~/.config/starship/starship.toml` from `starship/starship.linux.toml` and `starship/config-schema.json`; Windows uses `starship/starship.windows.toml`. Dev mode symlinks the selected Starship files, not the whole directory.
